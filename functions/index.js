 ...
 */
exports.updateQueueStatus = onSchedule({schedule: "every 10 minutes", region: "asia-southeast1",
}, async (event) => {
    ...
    try {
        ...
        for (const doc of activeSchedulesSnapshot.docs) {
            ...
            if (schedule.queueOpenMinutes === null || schedule.queueCloseMinutes === null) {
                continue;
            }

            const scheduledDatetime = DateTime.fromJSDate(
                schedule.scheduledDatetime.toDate()).setZone("Asia/Kuala_Lumpur");
            ...
            const queueOpenTime = scheduledDatetime.minus({minutes: queueOpenMinutes});
            const queueCloseTime = scheduledDatetime.minus({minutes: queueCloseMinutes});
        ...
        if (schedulesUpdated > 0) {
            await batch.commit();
            console.log(`Successfully updated ${schedulesUpdated} schedules' queue status.`);
        } else {
 ...
 */
exports.saveNotification = onCall({region: "asia-southeast1"}, async (request) => {
    ...
    if (timestamp) {
        ...
        firestoreTimestamp = new admin.firestore.Timestamp(scheduledTime.seconds, scheduledTime.nanoseconds || 0);
    }

    if (!firestoreTimestamp) {
        console.warn("No valid client timestamp found. Using server time as a fallback.");
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", "Failed to save notification. See logs for details.");
    }
});
...
exports.sendNotification = onCall({region: "asia-southeast1"}, async (request) => {
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", "Firebase Messaging service failed.");
    }
});
